...
#Detect column width by summing rows and pixel values
din = numpy.asarray(image)
summs = numpy.sum(din.astype(float), axis=0)
magnitude = numpy.sqrt(numpy.einsum('...i,...i', summs, summs))

diff = numpy.abs(numpy.diff(magnitude))

normal = diff / numpy.max(diff)
normal[normal<0.25]=0
...
for x in normal:
   if(x!=0):
      distList.append(last)
      last=0
   last+=1

shred_width = int(scipy.stats.mode(distList)[0][0])
